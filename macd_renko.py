...
# Defining functions for the technical indicators namely ATR, MACD & Renko for Backtesting
def MACD(DF,a,b,c):
...
def renko_DF(DF):
    ...
    for i in range(1, len(renko_df["bar_num"])):
        if renko_df["bar_num"][i] > 0 and renko_df["bar_num"][i-1] > 0:
            renko_df.loc[i,"bar_num"] += renko_df["bar_num"][i-1]
        elif renko_df["bar_num"][i] < 0 and renko_df["bar_num"][i-1] < 0:
            renko_df.loc[i,"bar_num"]+=renko_df["bar_num"][i-1]
    renko_df.drop_duplicates(subset="date",keep="last",inplace=True)
    return renko_df

# Defining functions for the KPIs (Key Performance Indicators)
def CAGR(DF):
...
# Download historical data for top Tech Stocks
tickers = ["MSFT", "GOOG", "AMZN"]

ohlc_intraday = {}  # directory with ohlc(open-high-low-close) value for each stock
ts = TimeSeries(key="P7FEEGDXXECOBSTM", output_format="pandas")
